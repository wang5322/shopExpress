const ImagesClass = require("../models/images.model");
...
exports.create = (req, res) => {
  ...
  ImagesClass.create(item, (err, item) => {
    if (err)
      res.status(500).send({
        message:
          err.message ||
...
exports.findAll = (req, res) => {
  ...
  ImagesClass.getAll(sortOrder, (err, list) => {
    if (err)
      res.status(500).send({
...
exports.findOne = (req, res) => {
  ImagesClass.findById(req.params.id, (err, item) => {
    if (err) {
